...
> **Design system: see [docs/DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md) — all UI work must conform to its surface-scoped tokens.** The doc opens with a Surface map showing which token bucket each surface uses (admin / client / field). Token tables, primitive APIs, layout patterns, and the polish list follow. The summary below stays as a quick reference for admin work; client-portal redesign reaches for the cream / ink / soft-shadow scale documented in the Client-only sections. Typography is Inter only across all surfaces (per brand spec — Helvetica Neue / Inter, no display serif).
>
...
- **App driver: `pg` (node-postgres) via `drizzle-orm/node-postgres`, registered with `attachDatabasePool()` from `@vercel/functions` in `src/db/index.ts`.** This is deliberate and load-bearing — do NOT revert to postgres.js. Vercel Fluid suspends warm instances between requests; a long-lived TCP socket dies silently during the suspend and the next request after idle reuses the corpse and hangs to the 300s limit → 504. Diagnostic signature: `wait_event = ClientRead` in `pg_stat_activity` for minutes, 504 on first request after idle, immediate reload succeeds. `attachDatabasePool` closes idle clients in the suspend lifecycle so no socket survives to go stale. Timer configs (`idle_timeout`/`max_lifetime`), `max:1`, and `Promise.race` app-side timeouts were all tried and all fail — see the git history around this note. The pool sets `statement_timeout`/`query_timeout` as a real fail-fast backstop (they cancel/evict, unlike `Promise.race`). node-pg uses unnamed prepared statements, so no `prepare:false` is needed on the transaction pooler.
- The CLI scripts (`scripts/verify-migrations.ts`, `apply-migration-manual.ts`, `apply-manual-sql.ts`, `verify-db.ts`) still use **postgres.js** against `DIRECT_URL` — they run in Node, not on Vercel, so the freeze issue doesn't apply and `postgres` stays a dependency.
