...
| File | Purpose |
| ---- | ------- |
| `manual_profile_trigger.sql` | Installs `public.handle_new_user()` + `auth.users` trigger that auto-creates a `profiles` row for every new auth user. **Superseded by `manual_profile_trigger_default_client.sql`** for the role default — apply that one last. |
| `manual_profile_trigger_default_client.sql` | `CREATE OR REPLACE`s `public.handle_new_user()` to default a metadata-less new user to `role = 'client'` (was `'admin'`) — fail-safe least-privilege. Apply after / instead of the original. See docs/MIGRATIONS.md. |
