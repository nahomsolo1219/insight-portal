...
`bg-brand-teal-500`, `text-brand-teal-500`, `border-brand-teal-500`).
...
All canonical colors live in `globals.css` `@theme`. Reach for these via Tailwind
utilities — never paste hex codes into JSX.

**Primary teal** (sidebar header, page titles, primary actions):
...
**Status / semantic colors** (Tailwind defaults, used for status pills and feedback):
...
- **Inter** (sans, 300/400/500/600/700) → CSS var `--font-inter` → utility `font-sans` (default body).
- **DM Serif Display** (400) → `--font-dm-serif` → utility `font-display`. **Page titles only.**
